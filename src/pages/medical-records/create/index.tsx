...
function MedicalRecordCreatePage() {
  ...
  const formik = useFormik<MedicalRecordInterface>({
    initialValues: {
      date_of_record: new Date(new Date().toDateString()),
      diagnosis: '',
      prescription: '',
      notes: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="date_of_record" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Date Of Record
            </FormLabel>
            <DatePicker
              selected={formik.values?.date_of_record ? new Date(formik.values?.date_of_record) : null}
              onChange={(value: Date) => formik.setFieldValue('date_of_record', value)}
            />
          </FormControl>
